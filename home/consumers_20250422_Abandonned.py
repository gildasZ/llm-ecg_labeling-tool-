...
class ECGConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if not self.user.is_authenticated:
            ...
            # Initialize instance variables: channels extracted from xml/csv files, current file path, reset condition 
            ...
            try:
                ...
                Data_to_Send_User_ID  = {'User_id': self.User_name}
                await async_send_to_pipe_channel(
                    ...
                    value=Data_to_Send_User_ID 
                )
                logger.info(f"\n+++++ Sent Data to dpd.Pipe: {Data_to_Send_User_ID } for User: {self.User_name}\n")

            except asyncio.CancelledError:
                await self.close(code=1001)  # Indicates that the server is shutting down

    async def disconnect(self, close_code):
        # Use self.User_name which was set during connect
        if hasattr(self, 'User_name') and self.User_name:
            await self.channel_layer.group_discard(self.group_name, self.channel_name)
            logger.info(f"\nWebSocket disconnect called with close code {close_code} by {self.User_name}.\n")
        else:
            logger.info(f"\nWebSocket disconnect called with close code {close_code} by unidentified user (no User_name stored).\n")

    async def receive(self, text_data):
        # Ensure user context is still valid (might not be strictly needed if disconnect handles it)
        if not hasattr(self, 'User_name') or not self.User_name:
             logger.warning("Receive called but User_name not set. Ignoring.")
             return
        logger.info(f"\nDjango Received data through WebSocket from {self.User_name}: {text_data}.\n")
        ...
        if data_type == 'processCSV_getMODELS':
            ...
            logger.info(f"\n+++++ Converted path: {self.current_file_path}")
            
            # *** MODIFIED: Send FilePath with target_user_id ***
            Data_to_Send = {
                'target_user_id': self.User_name,
                'file_path': self.current_file_path # Use a clear key for the actual data
            }
            await async_send_to_pipe_channel(
                        channel_name = 'FilePath_Channel',  # Fixed channel name for the pipe
                        label = 'FilePath_label',  # Fixed label for the pipe
                        value = Data_to_Send)
            logger.info(f"\n+++++ Django sent 'FilePath_Channel' Channel data to dpd.Pipe: {json.dumps(Data_to_Send)}\n\ttargeted for {self.User_name}")
        ...
        #______________________________________________________________________________
            ...
            logger.info(f"\nDjango received \n-relative file path: {path_variable} \n-and selected model: {selected_model}\n")
            ...
            # *** MODIFIED: Send Path/Model data with target_user_id ***
            Data_to_Send = {
                'target_user_id': self.User_name,
                'File-path': path_variable,
                'SelectedModel': selected_model,
                'Click_Order': self.count_auto_label # Keep this if needed by Dash logic
            }
            await async_send_to_pipe_channel(
                        channel_name = 'Receive_Django_Message_Channel',  # Fixed channel name for the second pipe
                        label = 'Path_and_Model_label',  # Fixed label for the second pipe
                        value = Data_to_Send)
            logger.info(f"\n+++++ Django sent Message Channel data to ppd.Pipe: {json.dumps(Data_to_Send)}\n\tfor self.User_name = {self.User_name}")
            ...
            if action_var in ('refresh', 'undo'):
                ...
                if self.handle_condition:
                    if self.count_number == 0: 
                        ...
                        self.count_number = 0
                    # *** MODIFIED: Send Button Action with target_user_id ***
                    Data_to_Send = {
                        'target_user_id': self.User_name,
                        'Action': action_var,
                        'Click_Order': self.count_number
                    }
                    await async_send_to_pipe_channel(
                ...
                if self.handle_condition:
                    ...
                    Data_to_Send = {
                        'target_user_id': self.User_name,
                        'Action': action_var,
                        'Click_Order': data_var # This is the data to delete
                    }
                    await async_send_to_pipe_channel(
                                channel_name = 'This_Action_Channel',  # Fixed channel name for the second pipe
                                label = 'This_Action',  # Fixed label for the second pipe
                ...
                if self.current_file_path:
                    ...
                    logger.warning(f"Save action '{action_var}' requested but current_file_path is not set for user {self.User_name}.")
                    await self.send(text_data=json.dumps({
                                                        'type': 'Save_Feedback',
                                                        'Message': "Error: No file context available to save.",
                                                        'Status': False
                                                    }))
            else:
                logger.warning(f"\t\t\tThe received 'Action variable' is not valid. \n\t\t\taction_var = {action_var}\n")
        ...
        #______________________________________________________________________________
            ...
            # *** MODIFIED: Send Labels Update with target_user_id ***
            Data_to_Send = {
                'target_user_id': self.User_name,
                'labels': label_status # Use a clear key for the payload
            }
            await async_send_to_pipe_channel(
                        channel_name = 'Labels_status_Channel',
                        label = 'Labels_Display_Status',
                        value = label_status)
            logger.info(f"\n+++++ Django sent Message updated_labels_status data to dpd.Pipe: {json.dumps(Data_to_Send)}\n\tfor self.User_name = {self.User_name}\n\tin conditional elif data['type'] == 'labels_display_updated'")

        #______________________________________________________________________________
        else:
            logger.error(f"\nUnknown message type received from {self.User_name}: {data_type}\n")
            await self.send(text_data=json.dumps({'error': 'Unknown message type'}))

    # --- Below are Methods called BY Dash app via channel layer groups ---
    # These are already user-specific because the group name includes self.User_name
